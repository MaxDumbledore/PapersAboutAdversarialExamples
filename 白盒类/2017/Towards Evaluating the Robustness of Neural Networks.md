...
与攻击中使用的参数相关，α和β表示输入到softmax层时的变化大小。JSMA-Z在未经提炼的网络上工作，这可能令人惊讶，因为它将所有变化视为同等重要，不管它们对softmax输出的影响有多大。如果更改单个像素会将目标类别增加10，但也会将最不可能的类别增加15，则攻击不会增加该像素

回想一下，在温度T下蒸馏会导致Logit的值增大T倍。实际上，这放大了上面提到的次优性，因为极不可能但有微小变化的logit可能会导致攻击拒绝进行任何更改

快速梯度符号一开始失败的原因与L - BFGS失败的原因相同：梯度几乎总是零。然而，如果我们尝试同样的除法技巧，在将logit除以T之后再将其输入softmax函数，则会发生有趣的事情：蒸馏仍然有效[36]。我们无法解释这种现象。

### B.实施我们的攻击

当我们将我们的攻击应用于防御蒸馏网络时，我们发现蒸馏只提供了边际价值。我们在MNIST和CIFAR-10上重新实施了防御性蒸馏，如[39]所述，使用的模型与我们在上述评估中使用的模型相同。我们在温度T=100的情况下训练蒸馏模型，发现该值最有效[39]。

表六显示了我们对蒸馏的攻击。之前的所有攻击都没有找到敌对的例子。相比之下，我们的攻击在三个距离指标中都以100%的成功概率成功。

与表IV相比，蒸馏几乎没有增加任何价值：我们的L0和L2攻击性能稍差，而我们的L∞ 攻击的效果大致相同。我们所有的攻击都取得了100%的成功

---

![image-20220404163907353](Towards Evaluating the Robustness of Neural Networks.assets/image-20220404163907353.png)

表六.将我们的攻击应用于防御网络时的比较。与表IV中未蒸馏的网络进行比较。
...
在最初的研究中，人们发现提高温度会持续降低攻击成功率。在MNIST上，从T=1的成功率为91%，到T=5的成功率为24%，最后到T=100的成功率为0.5%

我们用改进的攻击重新实施了这个实验，以了解温度的选择如何影响鲁棒性。我们训练温度从t=1到t=100的模型

当我们重新运行JSMA的实现时，我们观察到了相同的效果：攻击成功率迅速降低。然而，在我们改进的L2攻击中，我们看不到温度对对抗性示例的平均距离的影响：相关系数为ρ=−0.05. 这清楚地表明，提高蒸馏温度并不会增加神经网络的鲁棒性，它只会导致现有攻击更频繁地失败。

---

![image-20220404164258788](Towards Evaluating the Robustness of Neural Networks.assets/image-20220404164258788.png)

图8。MNIST上不同蒸馏温度的目标（随机目标）对抗性示例的平均距离。温度与平均对抗距离不相关

图9。针对目标（对抗类保持不变）和非目标（图像不是正确的类），对抗性示例从一个模型转移到另一个模型的概率。

---

### D.可迁移性

最近的研究表明，一个模型的对抗性示例通常会转化为另一个模型的对抗性示例，即使它们在不同的训练数据集上进行训练[46]，[11]，即使它们使用完全不同的算法（即，神经网络的对抗性示例转化为随机森林[37]）。

因此，任何能够针对对抗性例子提供稳健的辩护，都必须以某种方式打破这种可迁移性；否则，我们可以在一个易于攻击的模型上运行我们的攻击算法，然后将这些对抗性示例转移到难以攻击的模型中。

尽管防御蒸馏对我们更强的攻击并不强大，但我们通过将攻击从标准模型转移到防御蒸馏模型，展示了第二次蒸馏突破。

我们通过寻找高置信度的对抗性示例来实现这一点，我们将其定义为原始模型严重错误分类的对抗性示例。我们不想寻找一个只会将分类从源更改为目标的对抗性示例，而是想要一个目标代码比任何其他标签都更有可能的示例

回想一下前面为L2攻击定义的损失函数

![image-20220404164735751](Towards Evaluating the Robustness of Neural Networks.assets/image-20220404164735751.png)

参数κ的目的是控制对抗性示例的强度：κ越大，对抗性示例的分类越强。这使我们能够通过增加κ来生成高置信度的对抗性示例。

我们首先调查我们的假设是否成立，即第一个模型上的分类越强，转移的可能性就越大。我们通过将κ从0变为40来实现这一点。

我们的基线实验使用了两个在MNIST上训练的模型，如第四节所述，每个模型训练了一半的训练数据。我们发现，转移成功率从κ=0线性增加到κ=20，然后在κ成功率接近-100%时趋于平稳≈ 因此，明显增加κ增加了成功转移攻击的概率。

然后，我们运行相同的实验，知识用防御蒸馏训练第二个模型，并发现敌对的例子确实会转移。这为我们提供了另一种攻击技术，用于在经过提炼的网络上查找对抗性示例。

然而，有趣的是，与之前只需要κ=20的方法相比，无保护模型和蒸馏模型之间的转移成功率仅在κ=40时达到100%的成功率

**我们相信，这种方法通常可以用于评估防御的稳健性，即使防御能够完全阻止梯度流，从而导致我们基于梯度下降的方法无法成功。**



## 9. 结论

对抗性例子的存在限制了深度学习的应用领域。构建对对抗性示例具有鲁棒性的防御是一个公开的问题。为了解决这个问题，防御蒸馏被提出作为一种通用程序来提高任意神经网络的鲁棒性。

在本文中，我们提出了击败防御蒸馏的强大攻击，证明我们的攻击更普遍地可用于评估潜在防御的有效性。通过系统地评估许多可能的攻击方法，我们确定了一种比所有现有方法都能始终找到更好的对抗性示例的方法。我们使用这个评估作为我们的三个L0、L2和L的基础∞ 攻击

我们鼓励那些创建防御的人执行本文中使用的两种评估方法：

•使用强大的攻击（如本文提出的攻击）直接评估安全模型的鲁棒性。因为阻止我们的L2攻击的防御将阻止我们的其他攻击，所以防御者应该确保针对L2距离度量建立鲁棒性。

•通过在无担保模型上构建高置信度对抗性示例，并证明其无法转移到有担保模型，证明可转移性失败
